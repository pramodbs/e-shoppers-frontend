...
test.describe('Redis Search Integrity & Sync', () => {
  ...
  test('Sync on Create and Delete', async ({ page }) => {
    ...
    await page.getByLabel(/^Title$/i).fill(productName);
    await page.getByLabel(/Description/i).fill('Testing Redis Sync');
  ...
  test('Sync on Update', async ({ page }) => {
    ...
    await page.goto('/admin/products');
    await page.getByRole('button', { name: /Add/i }).click();
    await page.getByLabel(/^Title$/i).fill(originalName);
    ...
    await row.locator('button').first().click(); // Edit icon
    ...
    await expect(page.locator('.p-overlaypanel').getByText(updatedName)).toBeVisible({ timeout: 10000 });
    
    // Cleanup: Delete the updated product
    await page.goto('/admin/products');
    page.on('dialog', d => d.accept());
    await page.locator('tr').filter({ hasText: updatedName }).locator('button.MuiIconButton-colorError').click();
  });
